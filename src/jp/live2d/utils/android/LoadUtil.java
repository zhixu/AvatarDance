...
import com.live2d.avatardance.LAppDefine;
...
public class LoadUtil {
	...
	/*
	 * glGenTexturesã�®ä»£ã‚�ã‚Š
	 *
	 * Androidã�®ä¸€éƒ¨ã�§glGenTextures()ã�Œã�¨ã�³ã�¨ã�³ã�®ã�Šã�‹ã�—ã�ªå€¤ã‚’è¿”ã�—ã€�
	 * å ´å�ˆã�«ã‚ˆã�£ã�¦è² ã�®ç•ªå�·ï¼ˆuintã�§ã�‚ã‚Œã�°åˆ©ç”¨å�¯èƒ½ï¼Ÿï¼‰ã‚’è¿”ã�™ã�Ÿã‚�ã€�æ­£ã�®å€¤ã�Œå�–ã‚Œã‚‹ã�¾ã�§ç¹°ã‚Šè¿”ã�™
	 * ã‚¨ãƒ©ãƒ¼ã�®å ´å�ˆã�¯ï¼�
	 * @param gl
	 * @return
	 */
	public static int genTexture(GL10 gl)
	{
		...
		for( ; i< GEN_TEX_LOOP ; i++ )// äºˆæœŸã�›ã�¬ç„¡é™�ãƒ«ãƒ¼ãƒ—ã‚’é˜²æ­¢
		{
			int [] ret = {0} ;
			...
			if( texture < 0 )
			{
				gl.glDeleteTextures(1, ret, 0) ;// ä½œæˆ�ã�—ã�Ÿã‚‚ã�®ã‚’ç ´æ£„ã�™ã‚‹
			}
			else
			{
				break ;
	...
	public static int buildMipmap(GL10 gl, Bitmap srcBitmap , boolean recycle )
	{
		...
		try
		{
			// ã�“ã�®ä¸€æ–‡ã�Œã�ªã�„ã�¨ã€�Lynxã�§å´©ã‚Œã‚‹
		...
		while (height >= 1 && width >= 1)
		{
			...
			// the according level
			//  Lynx3Dã�§ã�¯level 0 ä»¥å¤–ã�§å‘¼ã�³å‡ºã�—ã�Ÿã�¨ã��ã�«ã�†ã�¾ã��ã�„ã�‹ã�ªã�„ã�“ã�¨ã�Œã�‚ã‚‹ã€‚
	...
	/*
	 * ã‚¢ã‚»ãƒƒãƒˆã�‹ã‚‰ãƒ‡ãƒ¼ã‚¿ã‚’èª­ã�¿è¾¼ã‚€ã€‚
	 * @param context
	 * @param filename
	 * @return
	 */
